...
# ---------------------------------------------------
    ...
    def lancerconversion(self):
        ...
        mpb = ttk.Progressbar(self, orient="horizontal", length=200, mode="determinate")
        mpb.place(x=430,y=340)
        mpb["maximum"] = self.dico["numPDF"]
        mpb["value"] = 0
        ...
        for number in range(self.dico["numPDF"]):
            pdf_file = self.dico["listePDF"][number][0]
            ...
            if self.dico["listePDF"][number][1] == 1:  # if the condition is True --> it puts the listpage = [1] because it is a single page pdf. ( they cause problems ... )
                list_pages = [1]
                self.converter(pdf_file, dpi, typecvd, amountdalto, amounttransf, list_pages)

            else:
                self.converter(pdf_file, dpi, typecvd, amountdalto, amounttransf)

            mpb["value"] = number+1



    def converter(self, pdf_file, dpi, typecvd, amountdalto, amounttransf, list_pages=None):


        thread_conversion = Thread(target=V6_1PDFColorMatrix.main, args=(pdf_file, dpi, typecvd, amountdalto, amounttransf, list_pages))
    ...
    def lancertest(self):
        ...
        pdf_file = self.dico["listePDF"][0][0]
        typecvd = self.daltotype.get()
        dpi = self.dpiscale.get()
        page = []
