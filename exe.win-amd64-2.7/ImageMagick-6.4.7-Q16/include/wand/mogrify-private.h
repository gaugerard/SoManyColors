...
#define FinalizeImageSettings(image_info,image) \
{ \
  (void) DeleteImageOption(image_info,"compress"); \
  (void) DeleteImageOption(image_info,"delay"); \
  (void) DeleteImageOption(image_info,"dispose"); \
  (void) DeleteImageOption(image_info,"loop"); \
  (void) DeleteImageOption(image_info,"page"); \
  (void) DeleteImageOption(image_info,"tile-offset"); \
        ...
        if ((fire) != MagickFalse) \
          { \
            status&=MogrifyImages(image_stack[k].image_info,postfix,(int) \
              (i-j+1),(const char **) argv+j,&image_stack[k].image,exception); \
            image=image_stack[k].image; \
